  ...
  return (
    <div className="bg-[#D2D5D6] p-3 rounded-lg mt-10 w-full max-w-[1000px] mx-auto small:p-5 lg:px-0">
      <div className="flex justify-center mb-5">
        <button
          className={`font-bold text-white w-[180px] rounded-lg p-2 text-lg ${
      ...
      >
        {segurosVisiveis.map((seguro, index) => (
          <motion.div
            ...
            viewport={{ once: false }}
            key={index}
            className="flex relative flex-col items-center p-4 text-center rounded-lg shadow-md bg-[#F0F0F0] w-[150px] h-[170px] small:w-[200px] small:h-[190px] small:text-[14px] small:cursor-pointer lg:w-[240px] lg:h-[200px] hover:scale-[2] hover:shadow-lg hover:bg-[#f8f8f8]"
            onClick={() => handleOpenModal(seguro)}
          >
            <p className="small:mt-2">{seguro.icon}</p>
            <h3 className="mt-3 text-xl font-bold small:mt-6 small:text-2xl font-asap text-blueRICADI">
              {seguro.title}
            </h3>
            <p className="hidden font-maven-pro mt-2 text-justify text-[#02465D] font-semibold text-[14px] small:pt-2">
              {seguro.description}
            </p>
            <button className="absolute bottom-3 small:text-xl small:h-[40px] text-secondWhiteRICADI bg-blueRICADI font-asap font-bold w-full max-w-[80%] h-[30px] rounded-md mt-2 hover:scale-[1.01] shadow-md">
              Ver mais
            </button>
          </motion.div>
        ))}
